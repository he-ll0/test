...
class homeWindow: #Main window: Home Window Class
    ...
    def requestValidation(self): #Assigns form entries to variables
        ...
        def saveForm():
            tePortalRequestsFile = open('TePortalRequestsFile.txt', 'a')
        ...
        for i in requestList: #Presence check (Pavitra, 2020)
            if len(i) == 0:
                ...
                presenceCheck = True
        if (emailCheck and  presenceCheck and requestEmptyCheck) == True:
            saveForm()
...
class helpWindow(object): #Second Window: Help Window Class
    def __init__(self,master):
        ...
        self.helpDescriptionText= Label(master, text=[' Hello User, welcome to the Technical Engineer (TE) support Portal. \n'
                                                      ' This Portal is the new SharePoint Portal for the Technical Engineering Team. \n \n'
                                                      ' This tool will allow users to raise requests for support and submit technical queries to the TE Team.\n'
                                                      ' The TE Portal is replacing the existing mailbox (*************@vodafone.com),'
                                                      ' which was previously used for support requests and queries.\n \n'
                                                      ' Users (Order Workflow Managers, Project Managers etc) can submit requests within the tool and'
                                                      ' assign priority levels based on the urgency of the request. \n'
                                                      ' Please follow the headings on the welcome page to input a request, once every field has been completed, click the Submit Form button.\n'
                                                      ' If the request has been saved successfuly, you will be presented with a success message, you can then close the program.'], bg='#ffffff',font=('Calibri',14)).pack(pady=30,ipady=10, ipadx=10)
